<div class="min-h-screen bg-slate-50 flex flex-col font-sans select-none" x-data="{
    ...
    if (e.code === 'Digit1') $wire.selectLabel('good');
    if (e.code === 'Digit2') $wire.selectLabel('spoof');
    if (e.code === 'Digit3') $wire.selectLabel('abnormal');
    if (e.key === 's' || e.key === 'S') $wire.skipImage();
});">
        ...
        @if ($attendance)
            <main
                ...
                <div class="w-full max-w-xl h-[480px] bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden relative flex items-center justify-center p-2 transition-transform duration-200"
                    ...
                    @if ($selectedLabel)
                        <div class="absolute top-4 left-1/2 -translate-x-1/2 pointer-events-none">
                            <span
                                class="inline-block text-white font-black text-2xl tracking-widest px-6 py-2.5 rounded-2xl shadow-lg uppercase {{ $selectedLabel === 'good' ? 'bg-emerald-500 shadow-emerald-200' : ($selectedLabel === 'spoof' ? 'bg-red-500 shadow-red-200' : 'bg-amber-400 shadow-amber-200') }}">
                                {{ strtoupper($selectedLabel) }}
                            </span>
                        </div>
                    @endif
                </div>
                ...
                <div
                    ...
                    <button type="button" @click="scale = Math.max(0.5, scale - 0.2)"
                    ...
                    <button type="button" @click="scale = Math.min(3, scale + 0.2)"
                        class="hover:text-slate-800 text-sm font-bold p-1">+</button>
                    <span class="text-slate-300">|</span>
                    <button type="button" @click="scale = 1"
                        class="hover:text-slate-800 text-[11px] font-semibold">Reset</button>
                </div>
            </main>
            ...
            <aside
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <button type="button" wire:click="selectLabel('good')"
                        ...
                        <button type="button" wire:click="selectLabel('spoof')"
                            class="w-full text-left p-4 rounded-xl border transition-all duration-150 flex items-center justify-between outline-none
                        ...
                        <button type="button" wire:click="selectLabel('abnormal')"
                            class="w-full text-left p-4 rounded-xl border transition-all duration-150 flex items-center justify-between outline-none
                ...
                <div class="space-y-4">
                    <button type="button" wire:click="saveAndNext" {{ !$selectedLabel ? 'disabled' : '' }}
                        class="w-full py-3.5 px-4 font-bold rounded-xl text-xs tracking-wider transition-all shadow-sm border border-transparent flex items-center justify-center gap-2
                            {{ $selectedLabel ? 'bg-slate-800 text-white hover:bg-slate-900 cursor-pointer active:scale-[0.99]' : 'bg-slate-200 text-slate-400 cursor-not-allowed' }}">
                    ...
                    <div class="flex items-center justify-between px-1 text-[10px] font-bold text-slate-400">
                        ...
                        <button type="button" wire:click="skipImage"
